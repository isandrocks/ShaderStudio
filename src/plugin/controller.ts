...
figma.showUI(__html__, { themeColors: true, width: 825, height: 625 });

let currentRect: RectangleNode | null = null;

figma.ui.onmessage = async (msg: { type: string; imageData?: Uint8Array }) => {
  switch (msg.type) {
    ...
    case 'shader-error':
      figma.notify('Shader rendering error', { error: true });
      figma.closePlugin();
      break;
...
function createRectangle() {
  ...
  // Timeout fallback
  setTimeout(() => {
    if (currentRect && (currentRect.fills as readonly Paint[])[0]?.type === 'SOLID') {
